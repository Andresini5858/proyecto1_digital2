...
void __interrupt() isr(void){ //Interrupciones
    ...
    if (INTCONbits.T0IF == 1){ //chequear interrupción del Timer0
        ...
        if (recibido == 0){ //Si es color rojo
            PORTAbits.RA1 = 1; //encender puerto
            delay(10); // delay (tiempo en alto del pulso)
            PORTAbits.RA1 = 0; //apagar 
        }
        else if (recibido == 1){ //Si es color verde
            PORTAbits.RA1 = 1; //encender puerto
            delay(27); // delay (tiempo en alto del pulso)
            PORTAbits.RA1 = 0; //apagar 
        }
        else if (recibido == 2){//Si es color azul
            PORTAbits.RA1 = 1; //encender puerto
            delay(45); // delay (tiempo en alto del pulso)
            PORTAbits.RA1 = 0; //apagar 
        }
        else {
            PORTAbits.RA1 = 0; //No hacer nada
        }
